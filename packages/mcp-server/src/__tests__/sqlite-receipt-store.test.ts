...
import { SqliteReceiptStore } from '../storage/sqlite-receipt-store.js'
...
describe('SqliteReceiptStore', () => {
  ...
  beforeEach(async () => {
    ...
    store = new SqliteReceiptStore(tmpDir)
    await store.init()
  })
  ...
  it('creates receipts.db file on init', () => {
    expect(existsSync(join(tmpDir, 'receipts.db'))).toBe(true)
  })
  ...
  it('filters by date range (from/to)', async () => {
    await store.save(makeReceipt({ receipt_id: 'rcpt_d1', timestamp: '2026-01-01T00:00:00.000Z' }))
    await store.save(makeReceipt({ receipt_id: 'rcpt_d2', timestamp: '2026-02-01T00:00:00.000Z' }))
    await store.save(makeReceipt({ receipt_id: 'rcpt_d3', timestamp: '2026-03-01T00:00:00.000Z' }))
    const result = await store.list({ from: '2026-01-15T00:00:00.000Z', to: '2026-02-15T00:00:00.000Z' })
    expect(result.data.length).toBe(1)
    expect(result.data[0]!.receipt_id).toBe('rcpt_d2')
  })
  ...
  it('overwrites receipt with same ID (upsert)', async () => {
  ...
  it('cleans up expired receipts', async () => {
    const past = new Date(Date.now() - 86400000).toISOString()
    const future = new Date(Date.now() + 86400000).toISOString()
    await store.save(makeReceipt({ receipt_id: 'rcpt_exp1', metadata: { expires_at: past } }))
    await store.save(makeReceipt({ receipt_id: 'rcpt_exp2', metadata: { expires_at: future } }))
    await store.save(makeReceipt({ receipt_id: 'rcpt_exp3', metadata: {} }))

    const result = await store.cleanup()
    expect(result.deleted).toBe(1)
    expect(result.total).toBe(3)
    expect(await store.exists('rcpt_exp1')).toBe(false)
    expect(await store.exists('rcpt_exp2')).toBe(true)
    expect(await store.exists('rcpt_exp3')).toBe(true)
  })

  it('migrates JSON files into SQLite on init', async () => {
    // Create a fresh tmp dir with JSON receipts
    const migrateDir = await mkdtemp(join(tmpdir(), 'sqlite-migrate-'))
    const receiptsDir = join(migrateDir, 'receipts')
    await mkdir(receiptsDir, { recursive: true })

    const r1 = makeReceipt({ receipt_id: 'rcpt_json1', action: 'migrated_action' })
    const r2 = makeReceipt({ receipt_id: 'rcpt_json2', action: 'migrated_action_2' })
    await writeFile(join(receiptsDir, 'rcpt_json1.json'), JSON.stringify(r1), 'utf-8')
    await writeFile(join(receiptsDir, 'rcpt_json2.json'), JSON.stringify(r2), 'utf-8')

    // Init a new SQLite store — should migrate
    const migrateStore = new SqliteReceiptStore(migrateDir)
    await migrateStore.init()

    const fetched1 = await migrateStore.get('rcpt_json1')
    expect(fetched1).not.toBeNull()
    expect(fetched1!.action).toBe('migrated_action')

    const fetched2 = await migrateStore.get('rcpt_json2')
    expect(fetched2).not.toBeNull()
    expect(fetched2!.action).toBe('migrated_action_2')

    const count = await migrateStore.count()
    expect(count).toBe(2)

    await rm(migrateDir, { recursive: true, force: true })
  })

  it('returns correct pagination keys', async () => {
    const result = await store.list({ status: 'completed' }, 1, 10, 'timestamp:desc')
    expect(Object.keys(result.pagination).sort()).toEqual([
      'has_next', 'has_prev', 'limit', 'page', 'total', 'total_pages',
    ])
  })

  it('filters by environment', async () => {
    await store.save(makeReceipt({ receipt_id: 'rcpt_e1', environment: 'production' }))
    await store.save(makeReceipt({ receipt_id: 'rcpt_e2', environment: 'test' }))
    const result = await store.list({ environment: 'production' })
    expect(result.data.length).toBe(1)
    expect(result.data[0]!.environment).toBe('production')
  })

  it('filters by receipt_type', async () => {
    await store.save(makeReceipt({ receipt_id: 'rcpt_rt1', receipt_type: 'action' }))
    await store.save(makeReceipt({ receipt_id: 'rcpt_rt2', receipt_type: 'judgment' }))
    const result = await store.list({ receipt_type: 'judgment' })
    expect(result.data.length).toBe(1)
    expect(result.data[0]!.receipt_type).toBe('judgment')
  })
})
